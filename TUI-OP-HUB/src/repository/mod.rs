//! Repository layer (US-CMD-01, US-CMD-03, US-PROJ-01, US-SRCH-01, US-SRCH-02).
use sqlx::SqlitePool;
use uuid::Uuid;
...
pub async fn create_entity(pool: &SqlitePool, req: &CreateEntity) -> AppResult<Entity> {
    ...
    sqlx::query(
        "INSERT INTO entities (id, name, description, content, type_id, project_id) VALUES (?, ?, ?, ?, ?, ?)"
    )
    .bind(&id)
    .bind(&req.name)
...
pub async fn update_entity(pool: &SqlitePool, id: &str, req: &CreateEntity) -> AppResult<Entity> {
    sqlx::query(
        "UPDATE entities SET name = ?, description = ?, content = ?, type_id = ?, project_id = ?, updated_at = datetime('now') WHERE id = ?"
    )
    .bind(&req.name).bind(&req.description).bind(&req.content)
    .bind(&req.type_id).bind(&req.project_id).bind(id)
    .execute(pool).await?;
    get_entity(pool, id).await
}
